...
Write-Host '== MiramarServer: OpenSSH + network fix =='
...
Write-Host ('WG adapter: '+((Get-NetConnectionProfile -EA 0|Where-Object{$_.InterfaceAlias -match 'TheGrid|WireGuard|wg'}|ForEach-Object{$_.NetworkCategory}) -join ','))
Write-Host ('AllowedIPs: '+((Get-Content "$env:ProgramData\TheGrid.conf" -EA 0|Select-String 'AllowedIPs')))