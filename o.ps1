...
Write-Host '== OpenSSH install (clock-proof) =='
[Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12
$d='C:\Program Files\OpenSSH'
function Fetch($u,$f){ try { Invoke-WebRequest $u -OutFile $f -UseBasicParsing; return $true } catch { return $false } }
if(-not (Test-Path (Join-Path $d 'sshd.exe'))){
  $z="$env:TEMP\osshg.zip"; $u='https://github.com/PowerShell/Win32-OpenSSH/releases/latest/download/OpenSSH-Win64.zip'
  if(-not (Fetch $u $z)){ Write-Host 'TLS failed (clock?) - retrying ignoring cert validation (one-time)'; [Net.ServicePointManager]::ServerCertificateValidationCallback={$true}; if(-not (Fetch $u $z)){ Write-Host 'DOWNLOAD FAILED - set the PC date/time correctly then re-run'; return } }
